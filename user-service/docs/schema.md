...
PostgreSQL 16. Source of truth: `migrations/*.sql`. GORM entities: `internal/models`.
...
|---|---|---|---|
| 001 | `users` | U1, U3 | `citext` email = case-insensitive unique |
| 002 | `allowed_email_domains` | U1.1.2 | empty table = no restriction |
| 002 | `auth_tokens` | U1.2, U2.1 | hash only; atomic consume (below); `requested_ip` used for rate limiting |
| 002 | `sessions` | U2.2, NFR-05.4 | opaque token hash; `revoked_at` for logout / logout-all |
| 003 | `roles` + `users.role` | U4, U5, U6 | one role per user; suspension is a role, so there's no status column |
| 003 | `admin_bootstrap` | U4.2 | singleton row; first signup wins it and becomes `super_admin` |
| 004 | `role_changes` | U4, U6 | append-only history; reason required when suspending/reinstating |
| 004 | `account_warnings` | U7 | `source_event_id` unique = idempotent; removed ⇔ `removed_at` set |
| 005 | `favourite_suppliers` | U3.4 | `supplier_id` has no FK (other service) |
...
- New schema change = new `00000N_name.up.sql` + `.down.sql`. Never edit an applied migration.
